...
(() => {
		...
		getGlobals() {
			...
			var globals = {
				...
				getCalculatedValue(key) {
					return env.getProperty(key).value;
				},
				references: []
			};
	...
	op.propertyKey = parameters.key;
	op.facetDefinition = facetDefinitions[parameters.key];
	if (op.facetDefinition) {
		result.value = op.displayText();
	}
